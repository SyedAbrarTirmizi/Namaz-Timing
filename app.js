...
const show_data = () => {
  let prev_data = JSON.parse(getCookie("tpt"));
  let magh_end_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
      local_date.getDate() +
      " " +
      prev_data.timings.Maghrib,
    0,
    30
  );

  let asr_end_date = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
  ...
  mag_end.innerHTML =
    magh_end_timestamp.getHours() + ":" + magh_end_timestamp.getMinutes();
...
const show_rem = (id, id_end_timestamp_ms, id_st_timestamp_ms) => {
  ...
  if (
    now_timestamp_ms > id_end_timestamp_ms ||
    now_timestamp_ms < id_st_timestamp_ms
  ) {
    ...
    let remining_time_m = Math.floor(
      (remining_time_ms - remining_time_h * (1000 * 60 * 60)) / (1000 * 60)
    );
    let remining_time_s = Math.floor(
      (remining_time_ms -
        remining_time_h * (1000 * 60 * 60) -
        remining_time_m * (1000 * 60)) /
        1000
    );
    id.innerHTML =
      remining_time_h + ":" + remining_time_m + ":" + remining_time_s;
    id.parentElement.parentElement.parentElement.style.cssText =
      "background:rgba(95, 53, 75, 1);color:#F4F4F4;";
    id.parentElement.parentElement.querySelector(
      "h4:nth-child(2) span"
    ).style.cssText = "background:#F7F7F7;padding:0 5px;";
  }
};
const calculate_remining = () => {
  let prev_data = JSON.parse(getCookie("tpt"));

  let fajr_st_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
      local_date.getDate() +
      " " +
      prev_data.timings.Fajr
  );
  let fajr_end_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
      local_date.getDate() +
      " " +
      prev_data.timings.Sunrise
  );

  let dhu_st_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
      local_date.getDate() +
      " " +
      prev_data.timings.Dhuhr
  );
  let dhu_end_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
      local_date.getDate() +
      " " +
      prev_data.timings.Asr
  );

  let asr_st_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
  ...
  let asr_end_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
  ...
  let magh_st_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
  ...
  let ish_st_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
      local_date.getDate() +
      " " +
      prev_data.timings.Isha
  );
  let ish_end_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
      local_date.getDate() +
      " " +
      prev_data.timings.Midnight
  );

  if (prev_data.timings.Midnight <= "11:59PM") {
    var tah_st_timestamp = add_h_to_date(
      local_date.getFullYear() +
        "-" +
        (local_date.getMonth() + 1) +
        "-" +
  ...
  let tah_end_timestamp = add_h_to_date(
    local_date.getFullYear() +
      "-" +
      (local_date.getMonth() + 1) +
      "-" +
  ...
  setInterval(() => {
    show_rem(
      fajr_rem,
      fajr_end_timestamp.getTime(),
      fajr_st_timestamp.getTime()
    );
    ...
    show_rem(
      mag_rem,
      magh_end_timestamp.getTime(),
      magh_st_timestamp.getTime()
    );
...
if (getCookie("tpt") === null) {
  ...
  data_btn.onclick = () => {
    if ("geolocation" in navigator) {
      ...
      alert(
        "Geolocation is not supported by this browser. Please use another browser"
      );
    }
  };
} else {
...
fetchImage(
  "https://images.pexels.com/photos/709552/pexels-photo-709552.jpeg?auto=compress&cs=tinysrgb&w=3504&h=2336&dpr=1",
  (url) => {
